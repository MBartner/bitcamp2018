...
function processData(){
	...
	if(command == 010){
		...
		handleUpdate(len, data);
	}
	else if(command == 0x40){
		handleLeave(len, data);
	}
	else{
...
function host(sessionName, callbackHost){
	...
	socket.send("\x01" + intToByteArray(len) + "\x01" + String.fromCharCode(sessionNameLen) + sessionName);
	state = 3;
...
function join(sessionName, callbackJoin, callbackUpdate){
	...
	sessionNameLen = sessionName.length;
	len = sessionNameLen + 1;
	socket.send("\x01" + intToByteArray(len) + "\x02" + String.fromCharCode(sessionNameLen) + sessionName);
	state = 1;
...
function sendUpdate(type, x, y, z){
	...
	socket.send("\x01" + intToByteArray(len)+ "\x03" + String.fromCharCode(type) + String.fromCharCode((x).toString().length) + (x).toString() + String.fromCharCode((y).toString().length) + (y).toString()+ String.fromCharCode((z).toString().length) + (z).toString());
	return 0;
}
...
function leave(){
	...
	socket.send("\x01" + intToByteArray(len) + "\x04" + String.fromCharCode(sessionNameLen) + sessionName);
	return 0;
}
